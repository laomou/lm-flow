 ...
 *
 *  2. **异常不得穿越 FFI**
 *     Python 异常在蹦床里捕获并转成错误码 + flow_ctx_set_error,绝不让它逃出 extern "C"。
 *
 *  3. **解释器生命周期**
 ...
 * 大块数值数据走 FlowBuffer,与 numpy 零拷贝互通。
 */
...
void check(FlowStatus st, const char* what) {
  if (st == FLOW_OK) return;
  const char* detail = flow_last_error();
  ...
  switch (st) {
    ...
    case FLOW_ERR_INVALID_ARG:
    case FLOW_ERR_UNSUPPORTED:
      throw py::value_error(msg);
    case FLOW_ERR_NOT_FOUND:
...
int dtype_from_numpy(const py::dtype& dt) {
  if (dt.is(py::dtype::of<uint8_t>())) return FLOW_DTYPE_U8;
  if (dt.is(py::dtype::of<int8_t>())) return FLOW_DTYPE_I8;
  if (dt.is(py::dtype::of<uint16_t>())) return FLOW_DTYPE_U16;
  if (dt.is(py::dtype::of<int16_t>())) return FLOW_DTYPE_I16;
  if (dt.is(py::dtype::of<int32_t>())) return FLOW_DTYPE_I32;
...
py::dtype numpy_from_dtype(int dt) {
  switch (dt) {
    case FLOW_DTYPE_U8: return py::dtype::of<uint8_t>();
    case FLOW_DTYPE_I8: return py::dtype::of<int8_t>();
    case FLOW_DTYPE_U16: return py::dtype::of<uint16_t>();
...
/// 把 FlowBuffer 包成 numpy 视图(**零拷贝**)。
/// `owner` 让底层缓冲在数组存活期间不被释放。
py::array wrap_buffer(const FlowBuffer& b, const py::object& owner, bool writable) {
...
class Packet {
 public:
  Packet() { raw_ = FlowPacket{nullptr, 0, FLOW_TS_UNSET, nullptr, nullptr}; }
  explicit Packet(FlowPacket raw, bool owned) : raw_(raw), owned_(owned) {}
  ...
  FlowPacket surrender() {
    owned_ = false;
    return raw_;
  }

  const FlowPacket& raw() const { return raw_; }
  FlowPacket* raw_mut() { return &raw_; }
  ...
  std::string type_name() const { return flow_type_name(raw_.type_id); }

  std::string repr() const {
    const char* s = flow_packet_debug_string(&raw_);
  ...
  std::optional<int64_t> as_int() const {
    int64_t v = 0;
    return flow_packet_as_i64(&raw_, &v) ? std::optional<int64_t>(v) : std::nullopt;
  }
  std::optional<double> as_float() const {
    double v = 0;
    return flow_packet_as_f64(&raw_, &v) ? std::optional<double>(v) : std::nullopt;
  }
  std::optional<bool> as_bool() const {
    bool v = false;
    return flow_packet_as_bool(&raw_, &v) ? std::optional<bool>(v) : std::nullopt;
  }
  std::optional<std::string> as_str() const {
    const char* s = nullptr;
    return flow_packet_as_str(&raw_, &s) ? std::optional<std::string>(s) : std::nullopt;
  }
  std::optional<py::bytes> as_bytes() const {
    const void* d = nullptr;
    size_t n = 0;
    if (!flow_packet_as_bytes(&raw_, &d, &n)) return std::nullopt;
  ...
  py::array as_numpy(const py::object& self) const {
    FlowBuffer b{};
    if (!flow_packet_as_buffer(&raw_, &b)) {
      throw py::value_error("该包不是 FlowBuffer(需要用 new_buffer 或 from_numpy 构造)");
    }
  ...
  py::array make_mutable(const py::object& self) {
    FlowBuffer b{};
    check(flow_packet_make_mutable_buffer(&raw_, &b), "make_mutable");
    return wrap_buffer(b, self, /*writable=*/true);
  }

  // ---- 内建类型构造 ----
  static Packet* from_int(int64_t v, int64_t ts) {
    return new Packet(flow_packet_from_i64(v, ts), true);
  }
  static Packet* from_float(double v, int64_t ts) {
    return new Packet(flow_packet_from_f64(v, ts), true);
  }
  static Packet* from_bool(bool v, int64_t ts) {
    return new Packet(flow_packet_from_bool(v, ts), true);
  }
  static Packet* from_str(const std::string& v, int64_t ts) {
    return new Packet(flow_packet_from_str(v.c_str(), ts), true);
  }
  static Packet* from_bytes(const py::bytes& v, int64_t ts) {
    py::buffer_info info(py::buffer(v).request());
    return new Packet(flow_packet_from_bytes(info.ptr, static_cast<size_t>(info.size), ts), true);
  }
  ...
  static Packet* from_numpy(const py::array& a, int64_t ts) {
    ...
    if (arr.ndim() < 1 || arr.ndim() > FLOW_MAX_DIMS) {
    ...
    return new Packet(flow_packet_from_buffer(&b, ts), true);
  }
...
static FlowPacket to_flow_packet(const py::object& o, int64_t ts) {
  if (py::isinstance<Packet>(o)) {
    auto* p = o.cast<Packet*>();
    if (ts != FLOW_TS_UNSET) p->set_timestamp(ts);
    return p->surrender();
  }
  ...
  if (py::isinstance<py::bool_>(o)) return flow_packet_from_bool(o.cast<bool>(), ts);
  if (py::isinstance<py::int_>(o)) return flow_packet_from_i64(o.cast<int64_t>(), ts);
  if (py::isinstance<py::float_>(o)) return flow_packet_from_f64(o.cast<double>(), ts);
  if (py::isinstance<py::bytes>(o)) {
    std::unique_ptr<Packet> p(Packet::from_bytes(o.cast<py::bytes>(), ts));
    return p->surrender();
  }
  if (py::isinstance<py::str>(o)) {
    return flow_packet_from_str(o.cast<std::string>().c_str(), ts);
  }
  throw py::type_error(
...
class Contract {
 public:
  explicit Contract(FlowContract* c) : c_(c) {}
  size_t num_inputs() const { return flow_contract_num_inputs(c_); }
  size_t num_outputs() const { return flow_contract_num_outputs(c_); }
  size_t input_id(const std::string& tag, size_t i) const {
    return flow_contract_input_id(c_, tag.c_str(), i);
  }
  size_t output_id(const std::string& tag, size_t i) const {
    return flow_contract_output_id(c_, tag.c_str(), i);
  }
  std::string input_name(size_t i) const { return flow_contract_input_name(c_, i); }
  std::string output_name(size_t i) const { return flow_contract_output_name(c_, i); }
  void input_set_any(size_t i) { flow_contract_input_set_any(c_, i); }
  void output_set_any(size_t i) { flow_contract_output_set_any(c_, i); }
  void input_set_type(size_t i, uint64_t t) { flow_contract_input_set_type(c_, i, t); }
  void output_set_type(size_t i, uint64_t t) { flow_contract_output_set_type(c_, i, t); }
  void require_side_packet(const std::string& n) {
    flow_contract_require_side_packet(c_, n.c_str());
  }
...
class Context {
 public:
  explicit Context(FlowContext* c) : c_(c) {}

  size_t num_inputs() const { return flow_ctx_num_inputs(c_); }
  size_t num_outputs() const { return flow_ctx_num_outputs(c_); }
  size_t input_id(const std::string& tag, size_t i) const {
    return flow_ctx_input_id(c_, tag.c_str(), i);
  }
  size_t output_id(const std::string& tag, size_t i) const {
    return flow_ctx_output_id(c_, tag.c_str(), i);
  }
  size_t input_index(const std::string& n) const { return flow_ctx_input_index(c_, n.c_str()); }
  std::string input_name(size_t i) const { return flow_ctx_input_name(c_, i); }
  std::string output_name(size_t i) const { return flow_ctx_output_name(c_, i); }

  std::string node_name() const { return flow_ctx_node_name(c_); }
  std::string kernel_name() const { return flow_ctx_kernel_name(c_); }
  int close_reason() const { return static_cast<int>(flow_ctx_close_reason(c_)); }

  bool input_is_empty(size_t i) const { return flow_ctx_input_is_empty(c_, i); }
  bool input_is_done(size_t i) const { return flow_ctx_input_is_done(c_, i); }
  int64_t input_timestamp() const { return flow_ctx_input_timestamp(c_); }

  /// 借用输入包 —— 回调返回后失效,不要留存。
  Packet* input(size_t i) const { return new Packet(flow_ctx_input(c_, i), /*owned=*/false); }

  /// 取走输入包(所有权移交)。写时复制省拷贝的第一步。
  Packet* take_input(size_t i) { return new Packet(flow_ctx_take_input(c_, i), true); }

  void emit(size_t out, const py::object& value, std::optional<int64_t> ts) {
    flow_ctx_emit(c_, out, to_flow_packet(value, ts.value_or(FLOW_TS_UNSET)));
  }
  void forward(size_t in, size_t out) { flow_ctx_forward(c_, in, out); }
  void set_next_timestamp_bound(size_t out, int64_t b) {
    flow_ctx_set_next_ts_bound(c_, out, b);
  }
  ...
  // ---- 参数 ----
  bool has_option(const std::string& k) const { return flow_ctx_has_option(c_, k.c_str()); }
  int64_t option_int(const std::string& k, int64_t d) const {
    return flow_ctx_option_i64(c_, k.c_str(), d);
  }
  double option_float(const std::string& k, double d) const {
    return flow_ctx_option_f64(c_, k.c_str(), d);
  }
  bool option_bool(const std::string& k, bool d) const {
    return flow_ctx_option_bool(c_, k.c_str(), d);
  }
  std::string option_str(const std::string& k, const std::string& d) const {
    const char* v = flow_ctx_option_str(c_, k.c_str(), d.c_str());
    return v ? v : d;
  }
  std::string options_json() const {
    const char* s = flow_ctx_options_json(c_);
    return s ? s : "{}";
  }
  /// 必需参数:缺失或类型不符直接抛异常 —— 让配置错误当场暴露,而不是静默用默认值。
  int64_t require_option_int(const std::string& k) const {
    int64_t v = 0;
    check(flow_ctx_require_option_i64(c_, k.c_str(), &v), "require_option_int");
    return v;
  }
  double require_option_float(const std::string& k) const {
    double v = 0;
    check(flow_ctx_require_option_f64(c_, k.c_str(), &v), "require_option_float");
    return v;
  }
  std::string require_option_str(const std::string& k) const {
    const char* v = nullptr;
    check(flow_ctx_require_option_str(c_, k.c_str(), &v), "require_option_str");
    return v ? v : "";
  }
  std::vector<int64_t> option_int_array(const std::string& k) const {
    size_t n = flow_ctx_option_count(c_, k.c_str());
    std::vector<int64_t> out(n);
    if (n) flow_ctx_option_i64_array(c_, k.c_str(), out.data(), n);
    return out;
  }
  std::vector<double> option_float_array(const std::string& k) const {
    size_t n = flow_ctx_option_count(c_, k.c_str());
    std::vector<double> out(n);
    if (n) flow_ctx_option_f64_array(c_, k.c_str(), out.data(), n);
    return out;
  }

  // ---- side packet / 日志 / 计数器 ----
  bool has_side_packet(const std::string& n) const {
    return flow_ctx_has_side_packet(c_, n.c_str());
  }
  Packet* side_packet(const std::string& n) const {
    return new Packet(flow_ctx_side_packet(c_, n.c_str()), false);
  }
  void log(int level, const std::string& msg) const {
    flow_ctx_log(c_, static_cast<FlowLogLevel>(level), msg.c_str());
  }
  void set_error(const std::string& msg) const { flow_ctx_set_error(c_, msg.c_str()); }
  void counter_add(const std::string& n, int64_t d) const {
    flow_ctx_counter_add(c_, n.c_str(), d);
  }
...
FlowStatus py_invoke(void* self, FlowContext* ctx, const char* method) {
  if (!self) return FLOW_ERR_KERNEL;
  // 引擎线程回调进 Python:必须先拿 GIL
  py::gil_scoped_acquire gil;
  try {
    auto* obj = static_cast<py::object*>(self);
    if (!py::hasattr(*obj, method)) return FLOW_OK;  // open/close 可选
    Context pc(ctx);
    obj->attr(method)(py::cast(pc, py::return_value_policy::move));
    return FLOW_OK;
  } catch (py::error_already_set& e) {
    flow_ctx_set_error(ctx, e.what());
    e.restore();
    PyErr_Clear();
    return FLOW_ERR_KERNEL;
...
FlowStatus py_open(void* self, FlowContext* c) { return py_invoke(self, c, "open"); }
FlowStatus py_process(void* self, FlowContext* c) { return py_invoke(self, c, "process"); }
FlowStatus py_close(void* self, FlowContext* c) { return py_invoke(self, c, "close"); }

void py_get_contract(void* factory, FlowContract* out) {
  auto* reg = static_cast<PyKernelReg*>(factory);
  py::gil_scoped_acquire gil;
  try {
...
const FlowKernelVTable kPyVTable = {&py_create, &py_get_contract, &py_open,
...
void register_python_kernel(const std::string& name, const py::object& cls) {
  ...
  check(flow_register_kernel(name.c_str(), &kPyVTable, reg), "register_kernel");
}
...
class Input {
 public:
  ...
  void send(const py::object& value, std::optional<int64_t> ts) {
    FlowPacket p = to_flow_packet(value, ts.value_or(FLOW_TS_UNSET));
    FlowStatus st;
    {
      py::gil_scoped_release unlock;
      st = flow_input_send(h_, p);
    }
    check(st, "send");
  }
  bool try_send(const py::object& value, std::optional<int64_t> ts) {
    ...
    {
      py::gil_scoped_release unlock;
      st = flow_input_try_send(h_, p);
    }
    if (st == FLOW_ERR_WOULD_BLOCK) return false;
    check(st, "try_send");
    return true;
  }
  void close() { flow_input_close(h_); }
 ...
 public:
  ...
  py::object next(std::optional<double> timeout) {
    ...
    if (timeout.has_value()) {
      ...
      {
        py::gil_scoped_release unlock;
        st = flow_poller_next_timeout(h_, &out, ms);
      }
      if (st == FLOW_ERR_TIMEOUT) {
        throw py::error_already_set();  // 由下方 translator 转 TimeoutError
      }
      if (st == FLOW_ERR_CLOSED) return py::none();
      check(st, "poller.next");
    } else {
      bool ok;
      {
        py::gil_scoped_release unlock;
        ok = flow_poller_next(h_, &out);
      }
  ...
  py::object try_next() {
    ...
    {
      py::gil_scoped_release unlock;
      ok = flow_poller_try_next(h_, &out);
    }
    if (!ok) return py::none();
    return py::cast(new Packet(out, true), py::return_value_policy::take_ownership);
  }
...
class Graph {
 public:
  ...
  void init_from_yaml(const std::string& yaml) {
    check(flow_graph_init_from_yaml(g_, yaml.c_str()), "init_from_yaml");
  }
  void init_from_yaml_file(const std::string& path) {
    check(flow_graph_init_from_yaml_file(g_, path.c_str()), "init_from_yaml_file");
  }

  void set_side_packet(const std::string& name, const py::object& value) {
    check(flow_graph_set_side_packet(g_, name.c_str(), to_flow_packet(value, FLOW_TS_UNSET)),
          "set_side_packet");
  }
  ...
  void observe(const std::string& port, const py::function& fn) {
    ...
    check(flow_graph_observe(g_, port.c_str(), &observer_trampoline, slot), "observe");
  }

  void start() { check(flow_graph_start(g_), "start"); }
  ...
  void close_input(const std::string& port) {
    check(flow_graph_close_input(g_, port.c_str()), "close_input");
  }
  void close_all_inputs() { flow_graph_close_all_inputs(g_); }
  void cancel() { flow_graph_cancel(g_); }
  void pause() { flow_graph_pause(g_); }
  void resume() { flow_graph_resume(g_); }
  ...
  void wait_done(std::optional<double> timeout) {
    FlowStatus st;
    {
      ...
      st = timeout.has_value()
               ? flow_graph_wait_done_timeout(g_, static_cast<int64_t>(*timeout * 1000.0))
               : flow_graph_wait_done(g_);
    }
    check(st, "wait_done");
  }
  ...
  void wait_until_idle(std::optional<double> timeout) {
    FlowStatus st;
    {
      py::gil_scoped_release unlock;
      st = timeout.has_value()
               ? flow_graph_wait_until_idle_timeout(g_, static_cast<int64_t>(*timeout * 1000.0))
               : flow_graph_wait_until_idle(g_);
    }
    check(st, "wait_until_idle");
  }

  int state() const { return static_cast<int>(flow_graph_state(g_)); }
  std::string dump() const {
    const char* s = flow_graph_dump(g_);
    return s ? s : "";
  }
  std::string last_error() const {
    const char* s = flow_graph_last_error(g_);
    return s ? s : "";
  }
  size_t queue_depth(const std::string& p) const {
    return flow_graph_queue_depth(g_, p.c_str());
  }
  uint64_t dropped_count(const std::string& p) const {
    return flow_graph_dropped_count(g_, p.c_str());
  }
  int64_t counter_value(const std::string& n) const {
    return flow_graph_counter_value(g_, n.c_str());
  }
  size_t total_queued() const { return flow_graph_total_queued(g_); }
  std::vector<std::string> node_names() const {
    std::vector<std::string> v;
    for (size_t i = 0, n = flow_graph_num_nodes(g_); i < n; ++i) {
      v.emplace_back(flow_graph_node_name(g_, i));
    }
    return v;
  }
  py::dict node_stats(size_t i) const {
    ...
    if (!flow_graph_node_stats(g_, i, &st)) throw py::index_error("节点序号越界");
    py::dict d;
 ...
 private:
  static void observer_trampoline(void* user, FlowPacket pkt) {
...
py::tuple Context::new_buffer(const std::vector<int64_t>& shape, const py::object& dtype) {
  ...
  FlowPacket raw = flow_packet_new_buffer(static_cast<int32_t>(shape.size()), shape.data(), dt,
                                          FLOW_TS_UNSET, &b);
  if (!raw.payload) throw std::runtime_error(std::string("new_buffer 失败: ") + flow_last_error());
...
void log_trampoline(void* /*user*/, FlowLogLevel level, const char* msg) {
  py::gil_scoped_acquire gil;
...
PYBIND11_MODULE(_lmflow, m) {
  ...
  m.attr("ABI_VERSION") = FLOW_ABI_VERSION;
  m.attr("TS_UNSET") = FLOW_TS_UNSET;
  m.attr("TS_PRE_STREAM") = FLOW_TS_PRE_STREAM;
  m.attr("TS_POST_STREAM") = FLOW_TS_POST_STREAM;
  m.attr("TS_DONE") = FLOW_TS_DONE;
  m.attr("INVALID_ID") = static_cast<uint64_t>(FLOW_INVALID_ID);

  m.attr("CLOSE_NORMAL") = static_cast<int>(FLOW_CLOSE_NORMAL);
  m.attr("CLOSE_ERROR") = static_cast<int>(FLOW_CLOSE_ERROR);
  m.attr("CLOSE_CANCELLED") = static_cast<int>(FLOW_CLOSE_CANCELLED);

  m.def("abi_version", &flow_abi_version);
  m.def("register_builtin_kernels", &flow_register_builtin_kernels,
  ...
  m.def(
      ...
      [] {
        std::vector<std::string> v;
        for (size_t i = 0, n = flow_registered_kernel_count(); i < n; ++i) {
          v.emplace_back(flow_registered_kernel_name(i));
        }
        return v;
      },
      "已注册的算子名(含内置 C++ 算子)");
  m.def(
      ...
      [](const py::object& cb) {
        g_log_cb = cb;
        if (cb.is_none()) {
          flow_set_log_callback(nullptr, nullptr);
        } else {
          flow_set_log_callback(&log_trampoline, nullptr);
        }
      },
      py::arg("cb"), "设置日志回调 fn(level, msg);传 None 恢复静默");
  m.def("type_name", [](uint64_t id) { return std::string(flow_type_name(id)); });
  ...
  py::class_<Packet>(m, "Packet")
      ...
      .def_static("from_int", &Packet::from_int, py::arg("value"), py::arg("ts") = FLOW_TS_UNSET)
      .def_static("from_float", &Packet::from_float, py::arg("value"),
                  py::arg("ts") = FLOW_TS_UNSET)
      .def_static("from_bool", &Packet::from_bool, py::arg("value"),
                  py::arg("ts") = FLOW_TS_UNSET)
      .def_static("from_str", &Packet::from_str, py::arg("value"), py::arg("ts") = FLOW_TS_UNSET)
