...
> **297 个测试**(Rust 255 + soak 1 + doctest 3 + Python 38)全绿,TSan 硬门禁 0 竞态。Rust / C++ / Python 三种宿主的
...
## 13. 测试策略(已落地 297 个:Rust 255 + soak 1 + doctest 3 + Python 38;另有 3 个独立 C++ 测试)
...
- **`LMFLOW_TYPE_HOST_OBJECT` 未启用**(ADR #26):跨语言算子只收发内建类型。
  它现在是**主动拒绝**,不再是「没人生产它」的默认状态 —— 契约声明它在建图期报错、包带它
  在运行期报错,两处都给出替代方案(数值集合用 `BUFFER`,任意元数据用 `STR` 装 JSON)。
