...
⚠ **「线性管线全程零拷贝」目前只在同步执行下确定成立** —— 即节点挂在
`DelegatingExecutor`(交还宿主线程)上时。放在**线程池**上时它是**尽力而为、不保证**:
上游节点的 `Context` 输入槽只在**下次调用开头**才清,若下游已在另一个 worker 线程上开始
改写,引用数就还是 ≥ 2,CoW 静默退化成复制。实测(600 次单包三级管线):委托执行器 0 次
复制,4 线程池约 13% 会复制。

这不是本次「默认改成线程池」引入的 —— 任何 `executor:` 指向线程池的图从来如此
(见 §13.2 那条「输入槽残留引用」的旧账,当年只修到「不漏」,没修到「池上也零拷贝」)。
但既然默认执行器现在是池,**默认路径就落在这个尽力而为的分支上了**。要让它在池上也确定成立,
得让上游在 dispatch / `finish` 时就释放输入槽,而不是等下次调用 —— 那是独立的引擎改动。
