...
| 34 | **运行统计分级**(`stats: off/basic/full`,默认 `basic`);`watchdog_ms > 0` 时强制 `full` | `basic` 保留状态、错误、吞吐、队列与背压等低成本诊断；`full` 再启用每次回调计时、百分位、CoW 和执行器耗时；`off` 仅保留调度正确性必需状态。这样默认不再为每次 `process` 支付两次 `Instant::now()`，又不制造 Cargo feature / ABI 变体。旧 `stats_timing: true/false` 分别兼容为 `full/basic`。watchdog 依赖单次耗时，故会强制 `full` 并说明原因。 |
