...
**零输入口 source 节点(生成型算子)已支持**(见 §7.4):内核自产数据、`source_done()` 自报产完；无数据时可 `source_yield(delay)` 协作式让出 worker。源不能挂委托执行器(否则会独占宿主线程)。内置 `RangeSourceKernel` 为样板。
...
| 34 | **运行统计分级**(`stats: off/basic/full`,默认 `basic`);`watchdog_ms > 0` 或 `trace_capacity > 0` 时强制 `full` | `basic` 保留状态、错误、吞吐、队列与背压等低成本诊断；`full` 再启用每次回调计时、百分位、CoW 和执行器耗时；`off` 仅保留调度正确性必需状态。这样默认不再为每次 `process` 支付两次 `Instant::now()`，又不制造 Cargo feature / ABI 变体。旧 `stats_timing: true/false` 分别兼容为 `full/basic`。watchdog 与逐次调用 trace 都依赖单次耗时，故都会强制 `full` 并说明是哪一个触发的。 |
...
| 38 | **声明式源定速 `rate: N`(Hz)** | 源本要么内核自己写 sleep、要么灌爆下游。`rate` 让定速变成一行 YAML。当前实现把下一次 Source 调用放入执行器的延迟队列，worker 用同一 `Condvar` 定时唤醒，不为每次等待创建线程，也不在 worker 内 sleep。按本次实际开始时刻计算剩余周期，避免累积漂移 |
...
| 44 | **外部 `LMFlowBuffer` 在解引用前做完整描述符校验，且本版本只接受 CPU 内存** | `device`/`flags`/`reserved` 若静默忽略，会把 GPU 指针或未来布局当 CPU 地址读；`ndim > 8` 会在切片时越界，任意 strides 的乘加也可能溢出。故 `lmflow_packet_from_buffer` 先统一验证维数、dtype、尾部零值、零 stride、数据指针和整个可寻址偏移范围，再进行连续快拷或 N 维逐元素拷贝。负 stride/非连续 CPU 视图继续支持；无法从裸指针证明真实分配大小，调用方仍须保证描述的地址范围确实可读 |
| 45 | **外部 CPU buffer 可显式零拷贝移交所有权，释放回调绑定 owner 而非 data 指针** | 解码器、相机和张量运行时常已拥有分配，强制 `from_buffer` 再复制会造成隐性带宽成本。`lmflow_packet_adopt_buffer` 按值保存描述符，并在最后一个 Packet 引用释放时调用 `release_fn(user_data)`；单独的 `user_data` 能表达基址、池句柄或带引用计数的 owner，即使 `data` 指向负步长视图中部也可正确释放。失败不转移所有权；回调必须可在任意工作线程执行。可写且独占时 CoW 原地返回外部 strides，READONLY 或共享时复制为引擎连续缓冲 |
...
| 29 | **`max_in_flight > 1` 要求所属执行器线程数 > 1** | 单线程池与委托执行器(0 线程)的并行度恒为 1;默认池按核数开,故默认节点配它是合法的。判据看解析出的执行器,不看 YAML 里有没有写 `executor` |
...
**线程池上的线性管线也保持零拷贝。** 算子回调返回后，引擎立即释放本次 invocation
的输入槽；派发到下游后，先清除 producer 的 staging 引用，再唤醒下游 worker。这样下游
`take_input` + `make_mutable` 在唯一消费者的线性管线中稳定看到独占 payload。扇出仍然
保留共享引用并按 CoW 复制，保证兄弟分支互不污染。
...
7. `max_in_flight > 1` 却落在**只有一个线程**的执行器上(单线程池、或 0 线程的委托执行器,
   并行度恒为 1)→ 报错,不静默降级。默认池按核数开,故默认节点配它通常合法。
...
- **协作等待**:`source_yield(delay)` 表示“本次正常返回、暂时无数据，请至少 `delay` 后再调我”。本次输出先完成按序刷新并释放 context 槽，随后 Source 进入执行器延迟队列；等待期间不占 worker。`delay=0` 是协作式让出，`source_done()` 与 yield 同时出现时以完成为准。非 Source 调用它是运行期错误。
- **定速**:两条路。① 内核用 `source_yield(delay)` 表达外部数据暂不可用；② 声明式 `rate: N`(节点字段,Hz)保证相邻两次 `process` 至少隔 `1/N` 秒。`rate` 与显式 yield 共用延迟调度路径，取两者要求的较长等待，不在 worker 内 sleep。`rate` 只对源有效，且须为正的有限值。source **必须挂线程池执行器**(挂委托执行器仍可能由用户回调独占宿主线程，建图期拒绝)。
- **阻塞边界**:引擎无法把一个已经阻塞在用户 `process()` 内的 Source 自动变成非阻塞；读 socket、等锁或自行 sleep 仍会占住当前 worker。需要共享小线程池时，内核必须采用 `source_yield` 或 `rate` 的协作式模型。采用协作模型后，多个 Source 可以共享单线程池，不再按 Source 数量拒绝建图。
...
- `block`:满时阻塞产出该边的线程,直到宿主取走一包。无损,但**宿主必须从另一线程并发排水**,
  ...
  > 就跑在宿主自己的线程上(委托执行器,或 `send` 直接派发)。那时宿主既是生产者又是唯一
...
内部背压状态转换也会进入统一日志回调。第 1、2、4、8… 次阻塞打 WARN,包含生产者、
消费节点/端口、有效容量、已排队/已预留/本次待入队包数;对应阻塞解除时打 INFO 并报告
持续时间。指数限频避免长期慢消费者刷屏。若最终无法推进,等待接口的错误会列出每条
`producer -> consumer.port` 阻塞关系及容量、排队、预留和已阻塞时间。

**阻塞 `send` 命中全局水位时怎么等** —— 分两种执行模型:

- 图跑在**委托执行器**(交还宿主线程):send 就地 `pump_step()` 自己推进一步(跑一个节点),
  把水位压下去再继续。
...
Compact / Diagnostics 的执行器图例同时显示当前 queued、running/线程容量、peak queued
与 completed；Topology 保持静态执行器形状。统计直接进入 DOT，不新增独立 Executor
stats 公共 API。
...
统计视图还维护**视图私有的区间基线**:首次导出统计本轮 `start` 至当前,之后按同一
`Compact` / `Diagnostics` 视图的相邻两次导出计算增量,两种视图互不污染,`reset`
清空基线。节点保留累计值,同时显示区间 processed、吞吐率、in/out 增量与区间平均耗时;
诊断边和 Poller 显示区间背压事件/时长与丢包增量。端口直接标注推断原因:消费者队列满、
...
nodes:
  ...
  - { name: "draw",   kernel: "Overlay" }        # 未指定 → 默认执行器(default)
```
...
**执行器类型** —— `executors[].type` 目前两种:

| `type` | 拥有线程? | 语义 |
|---|---|---|
| `ThreadPoolExecutor`(默认,`type` 留空也算它) | 是 | 自有 worker 线程,真并发 |
| `DelegatingExecutor` | **否** | 把就绪节点**交还宿主线程**跑;`num_threads`/`affinity`/`priority` 对它没有意义,写了报错而非静默忽略 |
...
- ⚠ 代价:默认执行不再零并发、**执行顺序不再确定**;且 Python 算子会在 worker 线程上
  抢 GIL(见 §8.2)。线程池上的线性 CoW 由 invocation 完成路径保证零拷贝；扇出共享时仍
  按 CoW 复制以隔离兄弟分支(见 §3.4)。
...
- 若宿主只 `send` 而从不调用上述任一接口,**委托执行器上的节点不会推进**。
  ...
  **每张图只有一个回调槽,再装是替换而非追加**;通知也是全图的(只说"某处有进展",
  不说哪个端口)。所以一个宿主里若有多处要被唤醒,须自己建一层分发:一个持有者装回调、
  广播给各等待者,各自重查条件。第二次安装会静默冲掉前一个,症状是图停止推进、无错无日志。
...
- 源节点(0 输入)**不能**挂委托执行器:用户 `process` 仍可能阻塞并独占宿主线程、
  ...
  协作 Source 不占等待中的 worker，因此多个 Source 可共享单线程池。
- 节点引用了未定义的 executor 名字 → init 阶段报错(§6.2 第 5 条)。

> ⚠ 实现注记:委托执行器的待办队列**由它自己持有**,不是 `GraphInner` 上的特例字段。
> 于是 `Node::executor` 只是一个普通下标,派任务的地方(`dispatch_task`)对两种执行器
> 一视同仁 —— 宿主线程在调度器眼里就是又一个执行器。
>
> 但 `pump_step` 的另一半 `try_advance_closing` **与执行器无关、不能省**:`maybe_close`
> 不向下游递归,已排空的 sink 只能靠宿主 pump 的全量扫描去 close 并置 `Terminated`。
> 纯线程池的图今天也依赖它。
...
默认 `sync` 已经按时间戳对齐全部输入口。只有需要“组间互不等待”时才应使用
`sync_set`;单个 set 覆盖全部输入口与默认 `sync` 等价,建议直接省略。某一组触发时,
其它组的输入不会作为残留值传入,而是继续留在各自队列中等待后续调用。
...
| **M2** ✅ | 委托执行器(当时是默认)+ 注册表 + YAML + Context 读写 + poller/observer | `hello_world` 输出 0..9(Rust 与 C++ 宿主各一份) |
...
| **M3** | 线程池 executor + 并发调度状态机 | 多线程下仍正确;TSan 干净 |
| **M6** | pybind11 绑定 + Python 算子 + 零拷贝 buffer + GIL 处理 | `examples/python/*` 跑通 |
| **A 阶段** | 时间戳同步、bound 传播、back-edge、并行 in-flight、输入策略 | 单列 |

> M3 之所以排在 M4/M5 之后:当年默认执行器是**宿主主线程**(旧 ADR #16),所以 M2 的
> 同步执行本就是产品的默认行为,而不是临时脚手架 —— 线程池当时是「显式 opt-in」的增量。
> 后来 ADR #16 反转:默认改成线程池,宿主线程执行降级为显式的 `DelegatingExecutor`
> (M2 那条路径至今仍在,只是不再是默认)。
...
- **无通用节点 timer**:执行器已有 Source 专用延迟唤醒(`source_yield` / `rate`)，
  但非源节点仍不能请求“每 N 毫秒给我一次 tick”。**未**设 `rate`、未调用
  `source_yield` 且持续产出的源仍会尽快运行，可能持续向下游施压。
