 ...
 *   - `LMFLOW_REGISTER_KERNEL(K)` 一行 self-register:文件作用域写一句即可
 *     (main 之前静态注册),无需集中登记。若算子被打进静态库，链接时必须保留
 *     完整 archive；官方 CMake targets 已自动处理。
 *
...
int main() {
  ...
  // 两个算子已由文件作用域的 LMFLOW_REGISTER_KERNEL 宏在 main 之前自注册。
