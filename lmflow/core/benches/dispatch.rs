...
//!
//! # 本机实测的噪声特征(2026-08-03,x86_64;换算成每包,BATCH=256)
//!
//! | 配置 | 每包 | 每跳边际 | 跑间波动 |
//! |---|---|---|---|
//! | `enqueue_only_paused` | ~77 ns | — | ±1% |
//! | `sink/main_thread` | 1484 ns | **763 ns** | ±1% |
//! | `sink/pool1` | 4281 ns | 2236 ns | ±1% |
//! | `sink/pool4` | 3402 ns | 401 ns | **±13%** |
//!
//! **`pool4` 波动大到不能用来归因代码改动**(4 线程抢 2 节点的锁 + 线程落位靠运气)——
//! 判断某次改动有没有效果,请看 `main_thread` / `pool1`,或给 `pool4` 大幅加采样数。
//! 曾因此误读过一次:一个纯删堆分配的改动在 `pool4` 上显示「回归 42%」,复测三次
//! 得到 880/830/945 µs —— 是噪声,不是回归。
//!
