...
/// `PassThrough` 无需任何注册调用就能用 —— 建图时由引擎自动注册(纯 Rust,零 C++)。
/// 这条同时钉住「默认算子在 --no-default-features 下也在」。
#[test]
fn default_passthrough_needs_no_registration() {
    let g = Graph::from_yaml(
        r#"
nodes:
...
/// 默认算子出现在注册表里(且不与 C++ 的 `PassThroughKernel` 撞名)。
#[test]
fn default_kernels_are_listed() {
