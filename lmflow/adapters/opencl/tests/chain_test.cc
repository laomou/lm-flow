// chain_test.cc —— OclAffine 数值 + 多算子 GPU 驻留链的测试。
//
// 与 adapters/vulkan/tests/chain_test.cc **同构**。要点有二:
//
//   1. **affine 数值**:out = in * scale + shift,对照 CPU 参考逐点比对,与 cpp/kernels/affine.cc
//      的 CPU 版同义。单独的 upload→affine→download 用例隔离它自己的正确性。
//   2. **多算子 GPU 驻留链**:OclUpload → OclResize → OclAffine → OclDownload。OclResize 与
//      OclAffine 之间的中间结果是一块 ocl::Image,**整条图只有末端一个 OclDownload** —— 中间
//      不落主机。这正是设备 buffer 池化(跨 dispatch 复用)开始回本的场景。
//
// 三个算子(OclUpload/OclResize/OclAffine + OclDownload)都由 lmflow_opencl_kernels 档案静态
// 自注册,本测试链的就是该档案,故图里直接引用、无需手写注册。
//
// 无 OpenCL 设备时优雅跳过(return 0),与其它 OpenCL 测试一致。
#include <cmath>
#include <cstdint>
#include <cstdio>
#include <cstdlib>
...
/// 把 host 数组包成一个 f32 的 LMFlowBuffer(HWC 连续)。
...
void RunAffineOnly(const char* label, int h, int w, int channels, double scale, double shift) {
  ...
  std::string yaml =
      std::string(
          ...
          "nodes:\n"
          "  - { name: up,   kernel: OclUpload,   executor: gpu, input_ports: [in], "
          "output_ports: [a] }\n"
          "  - { name: af,   kernel: OclAffine,   executor: gpu, input_ports: [a], "
          "output_ports: [b], options: { scale: ") +
      std::to_string(scale) + ", shift: " + std::to_string(shift) +
      " } }\n"
      "  - { name: down, kernel: OclDownload, executor: gpu, input_ports: [b], "
      "output_ports: [out] }\n"
      "input_ports: [in]\noutput_ports: [out]\n";
...
/// 多算子链:up → resize → affine → download。中间 resize→affine 的 ocl::Image 不落主机。
...
void RunChain(const char* label, int in_h, int in_w, int channels, int out_h, int out_w,
  ...
  std::string yaml =
      std::string(
          ...
          "  - { name: up,   kernel: OclUpload,   executor: gpu, input_ports: [in], "
          "output_ports: [a] }\n"
          "  - { name: rs,   kernel: OclResize,   executor: gpu, input_ports: [a], "
          "output_ports: [b], options: { out_h: ") +
      std::to_string(out_h) + ", out_w: " + std::to_string(out_w) +
      " } }\n"
      "  - { name: af,   kernel: OclAffine,   executor: gpu, input_ports: [b], "
      "output_ports: [c], options: { scale: " +
      std::to_string(scale) + ", shift: " + std::to_string(shift) +
      " } }\n"
      "  - { name: down, kernel: OclDownload, executor: gpu, input_ports: [c], "
      "output_ports: [out] }\n"
      "input_ports: [in]\noutput_ports: [out]\n";
...
int main() {
  ...
  std::printf("OclAffine 数值(out = in * scale + shift,对照 CPU 参考):\n");
  ...
  std::printf("多算子 GPU 驻留链(OclUpload→OclResize→OclAffine→OclDownload,中间不落主机):\n");
