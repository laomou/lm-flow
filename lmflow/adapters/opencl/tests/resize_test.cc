...
std::string GraphYaml(int out_h, int out_w) {
  return std::string(
             ...
             "nodes:\n"
             "  - { name: up, kernel: OclUpload, executor: gpu, "
             "input_ports: [in], output_ports: [a] }\n"
             "  - { name: rs, kernel: OclResize, executor: gpu, "
...
int main() {
  ...
  std::printf("OclResize 数值对照 CPU 参考(半像素中心双线性):\n");
  ...
  std::printf("%s\n", failures == 0 ? "lmflow_opencl_resize_test: all checks passed"
                                    : "lmflow_opencl_resize_test: FAILED");
