...
// 盈亏平衡在**连续 2~3 个 GPU 算子**时出现,本算子是链条中的一环。
//
// 但收益来自哪一半,已经实测过,和直觉不同(Adreno 740 / SM8550,resize→affine 预处理链,
// 见 adapters/vulkan/benchmarks/chain_bench.cc):
//
//   · **算子落点是主项**:同一个逐元素运算放 GPU 而不是放 CPU,快 17~24%,8 轮全为负。
//   · **「中间结果不落主机」只是次项**:两侧都用 GPU affine、只让中间结果多往返一次主机,
//     中位仅 -1.3% / -8.9% / -6.8%(中间结果 0.57 / 2.64 / 5.93 MB),而且**三档 8 轮全部
//     跨零** —— 这个量级在本机噪声内说不清。
//
// 原因是统一内存:那次"往返"其实是同一块物理内存里的两次 memcpy,不是过总线。所以在本项目
// 主打的移动端上,"驻留"省下的远没有"别用泛化 CPU 循环"多。
//
// ⚠ 这不推翻独显上的情形 —— 独显的回读要过 PCIe,往返代价应当大得多,但**手头没有可枚举的
// 独显**,那一档仍未实测。别把上面的结论外推到独显。
//
