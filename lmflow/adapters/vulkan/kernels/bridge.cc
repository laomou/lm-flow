// bridge.cc —— Vulkan CPU↔GPU 桥接算子的注册(注册名 "VkUpload" / "VkDownload")。
//
// 算子类本体在 <lmflow/vulkan.hpp>(UploadKernel / DownloadKernel):它们是纯 plumbing,
// 头文件**故意不自注册** —— 头会被多个 TU 包含,在头里放全局注册对象会到处重复注册。
// 注册的正确落点是随 adapter 发布的这个 kernels archive:一处静态自注册,图里即可直接写
// `kernel: VkUpload` / `kernel: VkDownload`,宿主不必再手写注册。
//
// 有了这两个桥接算子随 archive 落地,一条 CPU→GPU→CPU 的图才是**开箱即用**的:
//   VkUpload → VkResize → VkAffine → … → VkDownload
// 中间的 vk::Image 一路驻留设备、不落主机(设备 buffer 池化正为此服务)。
//
